...
## 💰 المرحلة 5 — إعلانات Google AdSense (خطوة بخطوة)

الكود جاهز بالكامل ومكانه الإعلانات محدد في 3 أماكن (الرئيسية، صفحة المقال، صفحات الأقسام).
سكريبت التحقق بتاع AdSense **ثابت في كود الصفحة نفسه** (مش بيتحمّل بجافاسكريبت لاحقًا)، عشان
روبوت مراجعة AdSense يقدر يشوفه فورًا من غير ما يحتاج يشغّل الموقع الأول.

### حسابك جاهز بالفعل — رقمك الحقيقي:
```
ca-pub-8726860784692880
```

### 1. ضيفيه كمتغير بيئة (خطوة واحدة بس المطلوبة منك دلوقتي)
- محليًا: في ملف `.env`، ضيفي:
  ```
  VITE_ADSENSE_CLIENT_ID=ca-pub-8726860784692880
  ```
- على Vercel: **Settings** → **Environment Variables** → أضيفي:
  - Name: `VITE_ADSENSE_CLIENT_ID`
  - Value: `ca-pub-8726860784692880`
  - **Save** ثم **Redeploy**.

### 2. ارفعي الكود الجديد
```bash
git add .
...
### 3. تأكدي إن الكود وصل للموقع الحقيقي
افتحي `animaljoystories.com`، كليك يمين → **View Page Source**، ودوري (`Ctrl+F`) عن
`adsbygoogle` — المفروض تلاقي سطر فيه `client=ca-pub-8726860784692880` واضح في الكود.

### 4. ارجعي لداشبورد AdSense واضغطي "Solicitar revisión" (طلب المراجعة)
دلوقتي بس، بعد ما تتأكدي إن الكود وصل فعليًا للموقع الحقيقي.

### 5. حدّثي ملف `ads.txt`
- افتحي `public/ads.txt` في المشروع.
- استبدلي السطر التجريبي بالسطر الحقيقي:
  ```
  google.com, pub-8726860784692880, DIRECT, f08c47fec0942fa0
  ```
  (لو AdSense وريتك كود مختلف شوية عن ده في صفحة "Fragmento de ads.txt"، استخدمي بالظبط
  اللي هي دتهولك).
- ده ملف إجباري، من غيره الإعلانات ممكن تترفض حتى بعد الموافقة.

### 6. الانتظار
بعد "Solicitar revisión"، جوجل بتاخد من **أيام لحد أسابيع** لمراجعة الموقع فعليًا وتوافق على
عرض الإعلانات. الإعلانات نفسها (المربعات) مش هتظهر للزوار غير بعد الموافقة الكاملة، حتى لو
الكود شغال.

### لو حبيتي تضيفي أماكن إعلانات جديدة لاحقًا
كل اللي محتاجاه هو سطر واحد في أي صفحة:
```jsx
<AdSlot slot="XXXXXXXXXX" />
```
(بعد ما تعملي "Ad unit" جديد في داشبورد AdSense وتاخدي رقم الـ slot بتاعه)
...
1. ~~لوحة التحكم~~ ✅ تمت.
2. ~~صفحة تفاصيل المقال~~ ✅ تمت.
3. ~~نظام تعليقات ومستخدمين~~ ✅ تمت.
4. ~~مرحلة التسييل: إعلانات~~ ✅ الكود جاهز، بانتظار موافقة AdSense.
5. ~~SEO~~ ✅ تمت.
6. ~~رفع الموقع أونلاين~~ ✅ تمت (Vercel).
7. مرحلة تسييل إضافية: روابط أفلييت حقيقية في قسم Best Finds، ميرتشاندايز.
